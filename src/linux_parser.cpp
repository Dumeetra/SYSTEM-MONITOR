...
{ 
  string a = "/proc/" + to_string(pid) + "/stat";
    string line, value;
    long jiffies{ 0 };
    int i{ 0 };
...
string LinuxParser::Command(int pid) {
  string line, value, myline;
            string a = "/proc/" + to_string(pid) + "/cmdline";
           std::ifstream filestream(a);
...
string LinuxParser::Ram(int pid) { 
        ...
        string key;
        string value;
        string a = "/proc/" + to_string(pid) + "/status";
        ...
        while (std::getline(filestream, line,':')) {
          std::istringstream linestream(line);
            while (linestream >> key) {
                ...
                else if (key == "0" && true1 == true) {
                    true2 = true;
                    true1 = false;
                }
                else if (key == "0" && true2 == true) {
                  return box;
                    break;
                }else{
                    box = key;
...
long LinuxParser::UpTime(int pid) {
        ...
        return trunc(box/sysconf( _SC_CLK_TCK ));                                  
 }
